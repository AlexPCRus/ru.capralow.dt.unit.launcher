 ...
 *******************************************************************************/
package ru.capralow.dt.internal.junit.launcher;
...
public class AssertionVMArg {
	...
	/**
	 * Sets default VM args in launch configuration to enable assertions if user
	 * preference indicates such, or blank if not.
	 *
	 * @param config
	 *            the launch configuration to default
	 */
	...
	public static String setAssertInArgString(String currentArgs) {
		...
		return (currentArgs.length() == 0) ? SHORT_VM_ARG_TEXT : currentArgs + " " + SHORT_VM_ARG_TEXT; //$NON-NLS-1$
	}
	...
	public static void setEnableAssertionsPreference(boolean preference) {
		InstanceScope.INSTANCE.getNode(JUnitCorePlugin.CORE_PLUGIN_ID)
				.putBoolean(JUnitPreferencesConstants.ENABLE_ASSERTIONS, preference);
	}

	/*
	 * not needed unless you're manipulating already entered configurations public
	 * static String removeAssertFromArgString(String currentArgs, String assertArg)
	 * { currentArgs.indexOf(assertArg); }
	 * 
	 * public static void syncAssertionVMArgInConfigs() { ILaunchManager lm=
	 * DebugPlugin.getDefault().getLaunchManager(); ILaunchConfigurationType
	 * configType=
	 * lm.getLaunchConfigurationType(JUnitLaunchConfiguration.ID_JUNIT_APPLICATION);
	 * try { ILaunchConfiguration[] configs=
	 * DebugPlugin.getDefault().getLaunchManager().getLaunchConfigurations(
	 * configType); for (int i=0; i<configs.length; ++i) { ILaunchConfiguration
	 * config = configs[i]; String vmArgs = config.getAttribute(
	 * IJavaLaunchConfigurationConstants.ATTR_VM_ARGUMENTS, ""); //$NON-NLS-1$
	 * String[] argArray = DebugPlugin.parseArguments(vmArgs); int assertArgIndex =
	 * findAssertEnabledArg(argArray); boolean assertAlreadyEnabled =
	 * (assertArgIndex != ASSERT_ARG_NOT_FOUND); if (assertAlreadyEnabled !=
	 * getEnableAssertionsPreference()) { if (assertAlreadyEnabled &&
	 * !getEnableAssertionsPreference()) { // remove the assertion argument vmArgs =
	 * removeAssertFromArgString(vmArgs, argArray[assertArgIndex]); } else if
	 * (!assertAlreadyEnabled && getEnableAssertionsPreference()) { // add the
	 * assertion argument vmArgs = setAssertInArgString(vmArgs); } // save whatever
	 * change was made ILaunchConfigurationWorkingCopy wc = config.getWorkingCopy();
	 * wc.setAttribute( IJavaLaunchConfigurationConstants.ATTR_VM_ARGUMENTS,
	 * vmArgs); wc.doSave(); } } } catch (CoreException e) { JUnitPlugin.log(e); }
	 * 
	 * }
	 */
}
