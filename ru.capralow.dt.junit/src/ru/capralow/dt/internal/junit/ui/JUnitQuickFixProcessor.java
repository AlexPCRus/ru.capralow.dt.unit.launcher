...
import ru.capralow.dt.internal.junit.BasicElementLabels;
import ru.capralow.dt.internal.junit.JUnitCorePlugin;
import ru.capralow.dt.internal.junit.Messages;
import ru.capralow.dt.internal.junit.util.ExceptionHandler;
import ru.capralow.dt.internal.junit.util.JUnitStubUtility;
...
public class JUnitQuickFixProcessor implements IQuickFixProcessor {
	...
	private static class AddAssertProposal implements IJavaCompletionProposal {
		...
		public String getAdditionalProposalInfo() {
			return Messages.format(JUnitMessages.JUnitQuickFixProcessor_add_assert_info,
					BasicElementLabels.getJavaElementName("org.junit.Assert." + fMethodName)); //$NON-NLS-1$
		}
	...
	private static class JUnitClasspathFixCorrectionProposal implements IJavaCompletionProposal {
		...
		public JUnitClasspathFixCorrectionProposal(IJavaProject javaProject, ClasspathFixProposal cpfix,
		...
		public void apply(IDocument document) {
			try {
				...
				ExceptionHandler.handle(e,
						JUnitPlugin.getActiveWorkbenchShell(),
						JUnitMessages.JUnitQuickFixProcessor_apply_problem_title,
						JUnitMessages.JUnitQuickFixProcessor_apply_problem_description);
		...
		protected Change createChange() throws CoreException {
			...
			if (fImportRewrite != null) {
				TextFileChange cuChange = new TextFileChange("Add import", //$NON-NLS-1$
	...
	public IJavaCompletionProposal[] getCorrections(final IInvocationContext context, IProblemLocation[] locations) {
		...
		for (IProblemLocation problem : locations) {
			...
			if (IProblem.UndefinedType == id) {
				...
				String currentPreferenceValue = PreferenceConstants.getPreferenceStore()
	...
	private ArrayList<IJavaCompletionProposal> addProposal(IInvocationContext context,
			ArrayList<IJavaCompletionProposal> proposals, String qualifiedName, IJavaProject javaProject) {
		ClasspathFixProposal[] fixProposals = ClasspathFixProcessor
				.getContributedFixImportProposals(javaProject, qualifiedName, null);
		for (ClasspathFixProposal fixProposal : fixProposals) {
			if (proposals == null)
				proposals = new ArrayList<>();
			proposals.add(new JUnitClasspathFixCorrectionProposal(javaProject,
					fixProposal,
					getImportRewrite(context.getASTRoot(), qualifiedName)));
		}
		return proposals;
	}
	...
	private ArrayList<IJavaCompletionProposal> getAddAssertImportProposals(IInvocationContext context,
			IProblemLocation problem, ArrayList<IJavaCompletionProposal> proposals) {
	...
	private ArrayList<IJavaCompletionProposal> getAddJUnitToBuildPathProposals(IInvocationContext context,
			IProblemLocation location, ArrayList<IJavaCompletionProposal> proposals) {
		try {
			...
			if (!(foundInProjectClasspath(javaProject, qualifiedName)
					&& foundInProjectClasspath(javaProject, qualifiedName1))) {
			...
			if (binding != null) {
				...
				for (IAnnotationBinding annotation : annotations) {
					...
					if (annotationType != null
							&& JUnitCorePlugin.JUNIT4_ANNOTATION_NAME.equals(annotationType.getQualifiedName()))
						return true;
				}
			}
		}
