...
import ru.capralow.dt.internal.junit.launcher.AssertionVMArg;
...
public class JUnitPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	...
	/**
	 * Create a new filter in the table (with the default 'new filter' value), then
	 * open up an in-place editor on it.
	 */
	...
	private void setDefaultValues() {
		fEnableAssertionsCheckBox.setSelection(DefaultScope.INSTANCE.getNode(JUnitCorePlugin.CORE_PLUGIN_ID).getBoolean(
				JUnitPreferencesConstants.ENABLE_ASSERTIONS,
				JUnitPreferencesConstants.ENABLE_ASSERTIONS_DEFAULT));
		fShowInAllViewsCheckBox.setSelection(DefaultScope.INSTANCE.getNode(JUnitPlugin.PLUGIN_ID).getBoolean(
				JUnitUIPreferencesConstants.SHOW_IN_ALL_VIEWS,
				JUnitUIPreferencesConstants.SHOW_IN_ALL_VIEWS_DEFAULT));
	...
	protected List<String> createInactiveStackFiltersList() {
		String[] strings = JUnitPreferencesConstants
				.parseList(getPreferenceStore().getString(JUnitPreferencesConstants.PREF_INACTIVE_FILTERS_LIST));
