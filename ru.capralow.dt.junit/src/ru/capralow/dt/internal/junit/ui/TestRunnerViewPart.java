...
import ru.capralow.dt.internal.junit.launcher.ITestKind;
import ru.capralow.dt.internal.junit.launcher.JUnitLaunchConfigurationConstants;
import ru.capralow.dt.internal.junit.launcher.TestKindRegistry;
import ru.capralow.dt.internal.junit.model.ITestRunSessionListener;
import ru.capralow.dt.internal.junit.model.ITestSessionListener;
import ru.capralow.dt.internal.junit.model.JUnitModel;
import ru.capralow.dt.internal.junit.model.TestCaseElement;
import ru.capralow.dt.internal.junit.model.TestElement;
import ru.capralow.dt.internal.junit.model.TestRunSession;
import ru.capralow.dt.junit.model.ITestElement.Result;
...
public class TestRunnerViewPart extends ViewPart {
	...
	private class DirtyListener implements IElementChangedListener {
		...
		private boolean processDelta(IJavaElementDelta delta) {
			...
			case IJavaElement.PACKAGE_FRAGMENT:
				// If we did something different than changing a child we flush the undo / redo
				// stack.
	...
	private class RunnerViewHistory extends ViewHistory<TestRunSession> {
		...
		public int getMaxEntries() {
			return Platform.getPreferencesService()
					.getInt(JUnitCorePlugin.CORE_PLUGIN_ID, JUnitPreferencesConstants.MAX_TEST_RUNS, 10, null);
		}
		...
		public String getText(TestRunSession session) {
			...
			if (session.getStartTime() <= 0) {
				...
				return Messages.format(JUnitMessages.TestRunnerViewPart_testName_startTime,
		...
		public void setMaxEntries(int maxEntries) {
			InstanceScope.INSTANCE.getNode(JUnitCorePlugin.CORE_PLUGIN_ID)
					.putInt(JUnitPreferencesConstants.MAX_TEST_RUNS, maxEntries);
		}
	}
					...
					if (JUnitUIPreferencesConstants.getShowInAllViews()
						...
						if (fInfoMessage == null) {
							String testRunLabel = BasicElementLabels
									.getJavaElementName(testRunSession.getTestRunName());
							String msg;
	...
	private class TestSessionListener implements ITestSessionListener {
		...
		public void testStarted(TestCaseElement testCaseElement) {
			...
			String status = Messages.format(JUnitMessages.TestRunnerViewPart_message_started,
	...
	static boolean getShowOnErrorOnly() {
		return Platform.getPreferencesService()
				.getBoolean(JUnitCorePlugin.CORE_PLUGIN_ID, JUnitPreferencesConstants.SHOW_ON_ERROR_ONLY, false, null);
	}
			...
			if (launchConfiguration != null) {
				try {
					...
					String oldFailuresFilename = launchConfiguration
							.getAttribute(JUnitLaunchConfigurationConstants.ATTR_FAILURES_NAMES, (String) null);
					String configName;
			...
			try { // show the result view
				...
				if (testRunner == null) {
					...
					testRunner = (TestRunnerViewPart) page
							.showView(TestRunnerViewPart.NAME, null, IWorkbenchPage.VIEW_VISIBLE);
					// restore focus
	...
	private void restoreLayoutState(IMemento memento) {
		...
		// if (page != null) {
		// int p= page.intValue();
		// if (p < fTestRunTabs.size()) { // tab count can decrease if a contributing
		// plug-in is removed
	...
	private void setTitleToolTip() {
		...
		if (testKindDisplayStr != null)
			setTitleToolTip(MessageFormat.format(JUnitMessages.TestRunnerViewPart_titleToolTip,
					new Object[] { testRunLabel, testKindDisplayStr }));
		else
	...
	private void showMessageIfNoTests() {
		if (fTestRunSession != null
			...
			Display.getDefault().asyncExec(new Runnable() {
				@Override
				public void run() {
					String msg = Messages.format(JUnitMessages.TestRunnerViewPart_error_notests_kind,
							fTestRunSession.getTestRunnerKind().getDisplayName());
					MessageDialog.openInformation(JUnitPlugin.getActiveWorkbenchShell(),
							JUnitMessages.TestRunnerViewPart__error_cannotrun,
							msg);
				}
			});
		}
