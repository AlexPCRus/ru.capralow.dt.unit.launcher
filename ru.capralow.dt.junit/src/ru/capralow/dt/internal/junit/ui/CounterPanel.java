	...
	public void setRunValue(int value, int ignoredCount, int assumptionFailureCount) {
		...
		if (ignoredCount == 0 && assumptionFailureCount == 0) {
			runString = Messages.format(JUnitMessages.CounterPanel_runcount,
					new String[] { Integer.toString(value), Integer.toString(fTotal) });
			runStringTooltip = runString;
		} else if (ignoredCount != 0 && assumptionFailureCount == 0) {
			runString = Messages.format(JUnitMessages.CounterPanel_runcount_skipped,
					new String[] { Integer.toString(value), Integer.toString(fTotal), Integer.toString(ignoredCount) });
			runStringTooltip = Messages.format(JUnitMessages.CounterPanel_runcount_ignored,
					new String[] { Integer.toString(value), Integer.toString(fTotal), Integer.toString(ignoredCount) });
		} else if (ignoredCount == 0 && assumptionFailureCount != 0) {
			runString = Messages.format(JUnitMessages.CounterPanel_runcount_skipped,
					new String[] {
			...
			runStringTooltip = Messages.format(JUnitMessages.CounterPanel_runcount_assumptionsFailed,
					new String[] {
							Integer.toString(value),
							Integer.toString(fTotal),
							Integer.toString(assumptionFailureCount) });
			...
			runStringTooltip = Messages.format(JUnitMessages.CounterPanel_runcount_ignored_assumptionsFailed,
					new String[] {
							Integer.toString(value),
							Integer.toString(fTotal),
