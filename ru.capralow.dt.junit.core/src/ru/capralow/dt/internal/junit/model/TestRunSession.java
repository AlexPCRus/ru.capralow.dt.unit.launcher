...
* 
*******************************************************************************/
package ru.capralow.dt.internal.junit.model;
...
import ru.capralow.dt.internal.junit.JUnitMessages;
...
import ru.capralow.dt.internal.junit.model.TestElement.Status;
import ru.capralow.dt.internal.junit.runner.MessageIds;
import ru.capralow.dt.junit.model.ITestElement;
import ru.capralow.dt.junit.model.ITestElementContainer;
import ru.capralow.dt.junit.model.ITestRunSession;
...
public class TestRunSession implements ITestRunSession {
	...
	public TestRunSession(ILaunch launch, IV8Project project, int port) {
		...
		if (launchConfiguration != null) {
			...
			fTestRunName = project.getProject().getName();
	...
	/*
	 * @see ru.capralow.dt.junit.model.ITestRunSession#getV8Project()
	 */
	@Override
	...
	/**
	 * @return <code>true</code> iff this session has been started, but not ended
	...
	private TestElement addTreeEntry(String treeEntry) {
		...
		if (isDynamicTest) {
			if (parentId != null) {
				for (IncompleteTestSuite suite : fFactoryTestSuites) {
					if (parentId.equals(suite.fTestSuiteElement.getId())) {
						return createTestElement(suite.fTestSuiteElement,
								id,
								testName,
								isSuite,
								testCount,
			...
			return createTestElement(getUnrootedSuite(),
					id,
					testName,
					isSuite,
					testCount,
			...
			if (fIncompleteTestSuites.isEmpty()) {
				return createTestElement(fTestRoot,
						id,
						testName,
						isSuite,
						testCount,
				...
				return createTestElement(openSuite.fTestSuiteElement,
						id,
						testName,
						isSuite,
						testCount,
	...
	public TestElement createTestElement(TestSuiteElement parent, String id, String testName, boolean isSuite,
		...
		if (isSuite) {
			...
			testElement = new TestCaseElement(parent,
					id,
					testName,
					displayName,
					isDynamicTest,
	...
	private TestSuiteElement getUnrootedSuite() {
		if (fUnrootedSuite == null) {
			fUnrootedSuite = (TestSuiteElement) createTestElement(fTestRoot,
					"-2", //$NON-NLS-1$
					JUnitMessages.TestRunSession_unrootedTests,
					true,
					0,
					false,
					JUnitMessages.TestRunSession_unrootedTests,
	...
	private class TestSessionNotifier implements ITestRunListener2 {
		...
		private TestElement createUnrootedTestElement(String testId, String testName) {
			...
			TestElement testElement = createTestElement(unrootedSuite,
					testId,
					testName,
					false,
					1,
		...
		if (fLaunch != null) {
			try {
				...
				if (launchConfig != null) {
					...
					if (hasIncludeTags) {
						return launchConfig.getAttribute(JUnitLaunchConfigurationConstants.ATTR_TEST_INCLUDE_TAGS,
								EMPTY_STRING);
					}
				}
			} catch (CoreException e) {
		...
		if (fLaunch != null) {
			try {
				...
				if (launchConfig != null) {
					...
					if (hasExcludeTags) {
						return launchConfig.getAttribute(JUnitLaunchConfigurationConstants.ATTR_TEST_EXCLUDE_TAGS,
								EMPTY_STRING);
					}
				}
			} catch (CoreException e) {
